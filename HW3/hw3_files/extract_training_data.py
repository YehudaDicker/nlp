...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        poses = self.pos_vocab

        words_inverted = {index: word for word, index in words.items()}
        pos_inverted = {index: pos for pos, index in poses.items()}
        
        stack = state.stack[-1:-4:-1][::-1] 
        buffer = state.buffer[-1:-4:-1][::-1] 
        #print("Stack:", stack)
        #print("Buffer:", buffer)
        #print(words)
        ...
        input_rep = []
        for index in stack:
            word = words_inverted.get(index, "<UNK>")
            if word ==  "<UNK>":
                input_rep.append(words[word])
            else:
                input_rep.append(index)

        while len(input_rep) < 3:
            input_rep.append(words["<NULL>"])

        for index in buffer:
            pos = pos_inverted.get(index, "<UNK>")
            if pos ==  "<UNK>":
                input_rep.append(poses[pos])
            else:
                input_rep.append(index)

        while len(input_rep) < 6:
            input_rep.append(poses["<NULL>"])

        #print("Input Representation:", input_rep)
    ...
    def get_output_representation(self, output_pair):  
        ...
        output_dict = self.output_labels

        output_index = output_dict[output_pair]

        vector[output_index] = 1

        #print("Print:", output_pair)
        #print("Vector:", vector)
